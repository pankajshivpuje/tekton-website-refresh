...
This document is aimed at helping Cluster Admins when configuring High Availability (HA) support for the Tekton Pipeline [Controller](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/controller.yaml) and [Webhook](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/webhook.yaml) components. HA support allows components to remain operational when a disruption occurs, such as nodes being drained for upgrades.
...
You can modify the replicas number in the [Controller deployment](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/controller.yaml) under `spec.replicas`, or apply an update to a running deployment:
...
Leader election can be configured in [config-leader-election.yaml](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/config-leader-election-controller.yaml). The ConfigMap defines the following parameters:
...
If HA is not required, you can disable it by scaling the deployment back to one replica. You can also modify the [controller deployment](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/controller.yaml), by specifying in the `tekton-pipelines-controller` container the `disable-ha` flag. For example:
...
In order to achieve HA for the Webhook deployment, you can modify the `replicas` number in the [Webhook deployment](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/webhook.yaml) under `spec.replicas`, or apply an update to a running deployment:

```sh
kubectl -n tekton-pipelines scale deployment tekton-pipelines-webhook --replicas=3
```

You can also modify the [HorizontalPodAutoscaler](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/config/webhook-hpa.yaml) to set a minimum number of replicas:
