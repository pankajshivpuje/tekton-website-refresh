...
- [Using `Workspaces` in `Pipelines`](/vault/Pipelines-v1.6.x-LTS/workspaces/#using-workspaces-in-pipelines)
- The [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/workspaces.yaml) code example
- The [variables available in a `PipelineRun`](/vault/Pipelines-v1.6.x-LTS/variables/#variables-available-in-a-pipeline), including `workspaces.<name>.bound`.
- [Mapping `Workspaces`](https://github.com/tektoncd/community/blob/main/teps/0108-mapping-workspaces.md)

## Specifying `Parameters`

(See also [Specifying Parameters in Tasks](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters))
...
by using [variable substitution](/vault/Pipelines-v1.6.x-LTS/variables/#variables-available-in-a-pipeline).
...
#### Param enum
> :seedling: **`enum` is an [alpha](/vault/Pipelines-v1.6.x-LTS/additional-configs/#alpha-features) feature.** The `enable-param-enum` feature flag must be set to `"true"` to enable this feature.
...
See usage in this [example](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/alpha/param-enum.yaml)

#### Propagated Params

Like with embedded [pipelineruns](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#propagated-parameters), you can propagate `params` declared in the `pipeline` down to the inlined `pipelineTasks` and its inlined `Steps`. Wherever a resource (e.g. a `pipelineTask`) or a `StepAction` is referenced, the parameters need to be passed explicitly. 
          ...
          - name: echo-action
            ...
            params:
              - name: msg
                value: "$(params.BYE)" 
```
The same rules defined in [pipelineruns](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#propagated-parameters) apply here.


## Adding `Tasks` to the `Pipeline`

 Your `Pipeline` definition must reference at least one [`Task`](/vault/Pipelines-v1.6.x-LTS/tasks/).
...
**Note:** Using both `apiVersion` and `kind` will create [CustomRun](/vault/Pipelines-v1.6.x-LTS/customruns/), don't set `apiVersion` if only referring to [`Task`](/vault/Pipelines-v1.6.x-LTS/tasks/).
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/vault/Pipelines-v1.6.x-LTS/resolution/). The below example demonstrates referencing a Task in git:
...
> :seedling: **Specifying `pipelines` in `PipelineTasks` is an [alpha](/vault/Pipelines-v1.6.x-LTS/additional-configs/#alpha-features) feature.**
...
---
```
For further information read [Pipelines in Pipelines](/vault/Pipelines-v1.6.x-LTS/pipelines-in-pipelines/)


### Specifying `Parameters` in `PipelineTasks`

You can also provide [`Parameters`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters):
...
> :seedling: **`Matrix` is an [beta](/vault/Pipelines-v1.6.x-LTS/additional-configs/#beta-features) feature.**
> The `enable-api-fields` feature flag can be set to `"beta"` to specify `Matrix` in a `PipelineTask`.

You can also provide [`Parameters`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters) through the `matrix` field:
...
For further information, read [`Matrix`](/vault/Pipelines-v1.6.x-LTS/matrix/).

### Specifying `Workspaces` in `PipelineTasks`

You can also provide [`Workspaces`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-workspaces):
...
You can reference a `Tekton bundle` in a `TaskRef` in both `v1` and `v1beta1` using [remote resolution](/vault/Pipelines-v1.6.x-LTS/bundle-resolver/#pipeline-resolution). The example syntax shown below for `v1` uses remote resolution and requires enabling [beta features](/vault/Pipelines-v1.6.x-LTS/additional-configs/#beta-features).
...
See the [Service Account](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#specifying-custom-serviceaccount-credentials) section
...
so long as the artifact adheres to the [contract](/vault/Pipelines-v1.6.x-LTS/tekton-bundle-contracts/).
...
been [cancelled](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#cancelling-a-pipelinerun) or
[gracefully cancelled](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#gracefully-cancelling-a-pipelinerun).
...
To specify `onError` for a `step`, please see [specifying onError for a step](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-onerror-for-a-step).
...
The declared `when` expressions are evaluated before the `Task` is run. If all the `when` expressions evaluate to `True`, the `Task` is run. If any of the `when` expressions evaluate to `False`, the `Task` is not run and the `Task` is listed in the [`Skipped Tasks` section of the `PipelineRunStatus`](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#monitoring-execution-status).
...
For an end-to-end example, see [PipelineRun with `when` expressions](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/pipelinerun-with-when-expressions.yaml).
...
> :seedling: **`CEL in WhenExpression` is an [alpha](/vault/Pipelines-v1.6.x-LTS/additional-configs/#alpha-features) feature.**
...
**Note:** To use Tekton's [variable substitution](/vault/Pipelines-v1.6.x-LTS/variables/), you need to wrap the reference with single quotes. This also means that if you pass another CEL expression via `params` or `results`, it won't be executed. Therefore CEL injection is disallowed.
...
**Note:** If you do not specify a `Timeout` value, Tekton instead honors the timeout for the [`PipelineRun`](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#configuring-a-pipelinerun).

**Note:** If the specified `Task` timeout is greater than the Pipeline timeout as configured in the [`PipelineRun`](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#configuring-a-failure-timeout), the Pipeline will time-out first causing the `Task` to timeout before its configured timeout.
...
See [`PipelineRun - Configuring a failure timeout`](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#configuring-a-failure-timeout) for details.

**Note:** Task timeouts specified in the Pipeline can be overridden at runtime using the [`taskRunSpecs`](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#overriding-individual-task-timeouts) field in the PipelineRun. This provides flexibility to adjust timeouts for specific execution contexts without modifying the Pipeline definition.
...
See the [complete list of variable substitutions for Pipelines](/vault/Pipelines-v1.6.x-LTS/variables/#variables-available-in-a-pipeline)
and the [list of fields that accept substitutions](/vault/Pipelines-v1.6.x-LTS/variables/#fields-that-accept-variable-substitutions).

For an end-to-end example, see [using context variables](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/using_context_variables.yaml).
...
Tasks can emit [`Results`](/vault/Pipelines-v1.6.x-LTS/tasks/#emitting-results) when they execute. A Pipeline can use these
...
[variable substitution](/vault/Pipelines-v1.6.x-LTS/variables/#variables-available-in-a-pipeline) - one `Task` emits
...
For an end-to-end example, see [`Task` `Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/task_results_example.yaml).
...
For an end-to-end example, see [`Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/pipelinerun-results.yaml).
...
For an end-to-end example see [`Array and Object Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/pipeline-emitting-results.yaml).
...
taskSpec](/vault/Pipelines-v1.6.x-LTS/taskruns/#specifying-the-target-task). For example:
...
`finally` tasks can specify [workspaces](/vault/Pipelines-v1.6.x-LTS/workspaces/) which `PipelineTasks` might have utilized
...
Similar to `tasks`, you can specify [`Parameters`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters) in `finally` tasks:
...
Similar to `tasks`, you can also provide [`Parameters`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters) through `matrix`
in `finally` tasks:
...
Please refer to the [table](/vault/Pipelines-v1.6.x-LTS/pipelineruns/#monitoring-execution-status) under Monitoring Execution Status to learn about
...
For an end-to-end example, see [`status` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
For an end-to-end example, see [`$(tasks.status)` usage in a `Pipeline`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/examples/v1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
Starting from `v0.47.0`, feature flag `custom-task-version` is removed and only `v1beta1` Custom Task will be supported. See the [migration doc](/vault/Pipelines-v1.6.x-LTS/migrating-v1alpha1.Run-to-v1beta1.CustomRun/) for details.
...
- [guidance for `Run`](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/docs/runs.md#developer-guide-for-custom-controllers-supporting-spec)
- [guidance for `CustomRun`](/vault/Pipelines-v1.6.x-LTS/customruns/#developer-guide-for-custom-controllers-supporting-customspec)
...
If a custom task supports [`parameters`](/vault/Pipelines-v1.6.x-LTS/tasks/#specifying-parameters), you can use the
...
[context variables](/vault/Pipelines-v1.6.x-LTS/variables/) that will be substituted, including:
...
> :seedling: **`Matrix` is an [alpha](/vault/Pipelines-v1.6.x-LTS/additional-configs/#alpha-features) feature.**
...
If the custom task supports it, you can provide [`Workspaces`](/vault/Pipelines-v1.6.x-LTS/workspaces/#using-workspaces-in-tasks) to share data with the custom task.
...
#### `v1alpha1.Run`
If the custom task supports it as [we recommended](https://github.com/tektoncd/pipeline/tree/release-v1.6.x/docs/runs.md#developer-guide-for-custom-controllers-supporting-timeout), you can provide `timeout` to specify the maximum running time of a `CustomRun` (including all retry attempts or other operations).

#### `v1beta1.CustomRun`
If the custom task supports it as [we recommended](/vault/Pipelines-v1.6.x-LTS/customruns/#developer-guide-for-custom-controllers-supporting-timeout), you can provide `timeout` to specify the maximum running time of one `CustomRun` execution.
