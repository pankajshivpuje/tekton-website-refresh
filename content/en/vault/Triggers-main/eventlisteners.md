...
It exposes an addressable sink that receives incoming event and specifies one or more [`Triggers`](/vault/Triggers-main/triggers/).
The sink is a Kubernetes service running the sink logic inside a dedicated Pod.

Each `Trigger`, in turn, allows you to specify one or more [`TriggerBindings`](/vault/Triggers-main/triggerbindings/) that allow
you to extract fields and their values from event payloads, and one or more [`TriggerTemplates`](/vault/Triggers-main/triggertemplates/)
...
or more [`Interceptors`](/vault/Triggers-main/interceptors/).
...
- A Kubernetes ClusterRoleBinding with `tekton-triggers-eventlistener-clusterroles` clusterrole.
  ...
  You can checkout an example [here](https://github.com/tektoncd/triggers/tree/main/examples/rbac.yaml).
...
- `triggerRef` - (optional) a reference to an external [`Trigger`](/vault/Triggers-main/triggers/)
...
For more information, see [`Interceptors`](/vault/Triggers-main/interceptors/).
...
An `EventListener` times out if it cannot process an event request within a timeout specified in [controller.yaml](https://github.com/tektoncd/triggers/tree/main/config/controller.yaml). The timeouts are as follows:
...
and our [TLS configuration example](https://github.com/tektoncd/triggers/tree/main/examples/v1beta1/eventlistener-tls-connection/README.md) for more information.
...
`ConfigMap` located in the `tekton-pipelines` namespace ([config-logging.yaml](https://github.com/tektoncd/triggers/tree/main/config/config-logging.yaml)).
...
You can configure metrics using the [`config-observability-triggers` config map](https://github.com/tektoncd/triggers/tree/main/config/config-observability.yaml) in the `EventListener` namespaces.
...
You can also use the Tekton [`create-ingress`](/vault/Triggers-main/getting-started/create-ingress.yaml) task to configure an `Ingress` object using self-signed certificates.
...
- How to securely control how each `EventListener` and its associated objects, such as [`Interceptors`](/vault/Triggers-main/interceptors/),
