...
A `PipelineRun` allows you to instantiate and execute a [`Pipeline`](/vault/Pipelines-v1.0.x-LTS/pipelines/) on-cluster.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifying-the-target-pipeline) - Specifies the target [`Pipeline`](/vault/Pipelines-v1.0.x-LTS/pipelines/).
- Optional:
  ...
  - [`taskRunSpecs`](#specifying-taskrunspecs) - Specifies a list of `PipelineRunTaskSpec` which allows for setting `ServiceAccountName`, [`Pod` template](/vault/Pipelines-v1.0.x-LTS/podtemplates/), and `Metadata` for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-v1.0.x-LTS/podtemplates/) to use as the basis for the configuration of the `Pod` that executes each `Task`.
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v1.0.x/examples/v1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v1.0.x/examples/v1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
in [Pipelines](/vault/Pipelines-v1.0.x-LTS/pipelines/#tekton-bundles) or [TaskRuns](/vault/Pipelines-v1.0.x-LTS/taskruns/#tekton-bundles).
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/vault/Pipelines-v1.0.x-LTS/resolution/). The below example demonstrates
...
See more details in [Param.Enum](/vault/Pipelines-v1.0.x-LTS/pipelines/#param-enum).
...
For more information, see [`ServiceAccount`](/vault/Pipelines-v1.0.x-LTS/auth/).

[`Custom tasks`](/vault/Pipelines-v1.0.x-LTS/pipelines/#using-custom-tasks) may or may not use a service account name.
...
You can specify a [`Pod` template](/vault/Pipelines-v1.0.x-LTS/podtemplates/) configuration that will serve as the configuration starting
...
[`Custom tasks`](/vault/Pipelines-v1.0.x-LTS/pipelines/#using-custom-tasks) may or may not use a pod template.
...
wide `ServiceAccountName`  and [`podTemplate`](/vault/Pipelines-v1.0.x-LTS/podtemplates/) configuration,
for example:
...
[Overriding `Task` `Steps` and `Sidecars`](/vault/Pipelines-v1.0.x-LTS/taskruns/#overriding-task-steps-and-sidecars) for more information.
...
- For information on mapping `Workspaces` to `Volumes`, see [Specifying `Workspaces` in `PipelineRuns`](/vault/Pipelines-v1.0.x-LTS/workspaces/#specifying-workspaces-in-pipelineruns).
- For a list of supported `Volume` types, see [Specifying `VolumeSources` in `Workspaces`](/vault/Pipelines-v1.0.x-LTS/workspaces/#specifying-volumesources-in-workspaces).
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.0.x/examples/v1/pipelineruns/workspaces.yaml).

[`Custom tasks`](/vault/Pipelines-v1.0.x-LTS/pipelines/#using-custom-tasks) may or may not use workspaces.
...
For more information, see the [`LimitRange` support in Pipeline](/vault/Pipelines-v1.0.x-LTS/compute-resources/#limitrange-support).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v1.0.x/config/config-defaults.yaml).
...
- Required:
  ...
  - [`pipelineSpec`](/vault/Pipelines-v1.0.x-LTS/pipelines/#configuring-a-pipeline) - The exact `PipelineSpec` used when starting the `PipelineRun`.
- Optional:
  - [`pipelineResults`](/vault/Pipelines-v1.0.x-LTS/pipelines/#emitting-results-from-a-pipeline) - Results emitted by this `PipelineRun`.
  - `skippedTasks` - A list of `Task`s which were skipped when running this `PipelineRun` due to [when expressions](/vault/Pipelines-v1.0.x-LTS/pipelines/#guard-task-execution-using-when-expressions), including the when expressions applying to the skipped task.
  - `childReferences` - A list of references to each `TaskRun` or `Run` in this `PipelineRun`, which can be used to look up the status of the underlying `TaskRun` or `Run`. Each entry contains the following:
    ...
    - [`whenExpressions`](/vault/Pipelines-v1.0.x-LTS/pipelines/#guard-task-execution-using-when-expressions) - The list of when expressions guarding the execution of this task.
...
to a `TaskRun`, the complete [status of the `TaskRun`](/vault/Pipelines-v1.0.x-LTS/taskruns/#monitoring-execution-status) and details
...
When a `PipelineRun` changes status, [events](/vault/Pipelines-v1.0.x-LTS/events/#pipelineruns) are triggered accordingly.
...
When a `PipelineRun` has `Tasks` with [`when` expressions](/vault/Pipelines-v1.0.x-LTS/pipelines/#guard-task-execution-using-when-expressions):
