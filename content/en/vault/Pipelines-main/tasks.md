...
- The container image must abide by the [container contract](/vault/Pipelines-main/container-contract/).
...
- The CPU, memory, and ephemeral storage resource requests set on `Step`s
  ...
  For more detail, see [Compute Resources in Tekton](/vault/Pipelines-main/compute-resources/).

**Note:** If the image referenced in the `step` field is from a private registry, `TaskRuns` or `PipelineRuns` that consume the task
          must provide the `imagePullSecrets` in a [podTemplate](/vault/Pipelines-main/podtemplates/).
...
* `/workspace` - This directory is where [resources](#specifying-resources) and [workspaces](#specifying-workspaces)
  are mounted. Paths to these are available to `Task` authors via [variable substitution](/vault/Pipelines-main/variables/)
* `/tekton` - This directory is used for Tekton specific functionality:
    * `/tekton/results` is where [results](#emitting-results) are written to.
      The path is available to `Task` authors via [`$(results.name.path)`](/vault/Pipelines-main/variables/)
    * There are other subfolders which are [implementation details of Tekton](https://github.com/tektoncd/pipeline/tree/main/docs/developers/README.md#reserved-directories)
...
For an end-to-end example, see [the taskRun ignoring a step error](https://github.com/tektoncd/pipeline/tree/main/examples/v1/taskruns/ignore-step-error.yaml)
and [the pipelineRun ignoring a step error](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/ignore-step-error.yaml).
...
[Debugging](/vault/Pipelines-main/taskruns/#debugging-a-taskrun) a taskRun is supported to debug a container and comes with a set of
[tools](/vault/Pipelines-main/taskruns/#debug-environment) to declare the step as a failure or a success. Specifying
[breakpoint](/vault/Pipelines-main/taskruns/#breakpoint-on-failure) at the `taskRun` level overrides ignoring a step error using `onError`.

#### Redirecting step output streams with `stdoutConfig` and `stderrConfig`

This is an alpha feature. The `enable-api-fields` feature flag [must be set to `"alpha"`](/vault/Pipelines-main/install/)
...
Refer to the [TaskRun example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/taskruns/object-param-result.yaml) and the [PipelineRun example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/pipeline-object-param-and-result.yaml) in which `object` parameters are demonstrated.
  ...
  > - When providing value for an `object` param, one may provide values for just a subset of keys in spec's `default`, and provide values for the rest of keys at runtime ([example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/taskruns/object-param-result.yaml)).
  > - When using object in variable replacement, users can only access its individual key ("child" member) of the object by its name i.e. `$(params.gitrepo.url)`. Using an entire object as a value is only allowed when the value is also an object like [this example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/pipeline-object-param-and-result.yaml). See more details about using object param from the [TEP-0075](https://github.com/tektoncd/community/blob/main/teps/0075-object-param-and-result-types.md#using-objects-in-variable-replacement).
...
[`Workspaces`](/vault/Pipelines-main/workspaces/#using-workspaces-in-tasks) allow you to specify
...
For more information, see [Using `Workspaces` in `Tasks`](/vault/Pipelines-main/workspaces/#using-workspaces-in-tasks)
and the [`Workspaces` in a `TaskRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1/taskruns/workspace.yaml) example YAML file.

### Propagated `Workspaces`

Workspaces can be propagated to embedded task specs, not referenced Tasks. For more information, see [Propagated Workspaces](/vault/Pipelines-main/taskruns/#propagated-workspaces).
...
The stored results can be used [at the `Task` level](/vault/Pipelines-main/pipelines/#passing-one-tasks-results-into-the-parameters-or-when-expressions-of-another)
or [at the `Pipeline` level](/vault/Pipelines-main/pipelines/#emitting-results-from-a-pipeline).
...
[`"sidecar-logs"`](/vault/Pipelines-main/additional-configs/#enabling-larger-results-using-sidecar-logs) to enable larger results using sidecar logs.
...
**CAUTION**: we need you to enable access to [kubernetes pod/logs](/vault/Pipelines-main/additional-configs/#enabling-larger-results-using-sidecar-logs).
...
`max-result-size` feature flag to your desired size in bytes ([see instructions](/vault/Pipelines-main/additional-configs/#enabling-larger-results-using-sidecar-logs)).
...
Refer to the detailed instructions listed in [additional config](/vault/Pipelines-main/additional-configs/#enabling-larger-results-using-sidecar-logs)
...
- [Mount a Kubernetes `Secret`](/vault/Pipelines-main/auth/).
...
See the [complete list of variable substitutions for Tasks](/vault/Pipelines-main/variables/#variables-available-in-a-task)
...
- To reference a parameter in a `Task`, use the following syntax, where `<name>` is the name of the parameter:
  ...
  $(params["<name>"])
  ```
- To access parameter values from resources, see [variable substitution](/vault/Pipelines-main/resources/#variable-substitution)
...
The example Task/TaskRun above can be found as a [TaskRun example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/taskruns/run-steps-as-non-root.yaml).
