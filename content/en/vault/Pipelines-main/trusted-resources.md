...
**Note**: Currently, trusted resources only support verifying Tekton resources that come from remote places i.e. git, OCI registry and Artifact Hub. To use [cluster resolver](/vault/Pipelines-main/cluster-resolver/) for in-cluster resources, make sure to set all default values for the resources before applied to cluster, because the mutating webhook will update the default fields if not given and fail the verification.
...
To learn more about `ConfigSource` please refer to resolvers doc for more context. e.g. [gitresolver](/vault/Pipelines-main/git-resolver/)
