...
Like with `Tasks`, a `StepAction` must declare all the parameters that it uses. The same rules for `Parameter` [name](/vault/Pipelines-main/tasks/#parameter-name), [type](/vault/Pipelines-main/tasks/#parameter-type) (including [object](/vault/Pipelines-main/tasks/#object-type), [array](/vault/Pipelines-main/tasks/#array-type) and [string](/vault/Pipelines-main/tasks/#string-type)) apply as when declaring them in `Tasks`. A `StepAction` can also provide [default value](/vault/Pipelines-main/tasks/#default-value) to a `Parameter`.
...
Note that the `securityContext` from `StepAction` will overwrite the `securityContext` from [`TaskRun`](/vault/Pipelines-main/taskruns/#example-of-running-step-containers-as-a-non-root-user).
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/vault/Pipelines-main/resolution/). The below example demonstrates referencing a `StepAction` in git:
...
The default resolver type can be configured by the `default-resolver-type` field in the `config-defaults` ConfigMap (`alpha` feature). See [additional-configs.md](/vault/Pipelines-main/additional-configs/) for details.
