...
You also have the option to define a global Pod template [in your Tekton config](/vault/Pipelines-main/additional-configs/#customizing-basic-execution-parameters) using the key `default-pod-template`.
...
When using Pod templates within `PipelineRun` [`taskRunSpecs`](/vault/Pipelines-main/pipelineruns/#specifying-taskrunspecs), you can use parameter substitution to dynamically configure Pod template fields based on pipeline parameters. This is particularly useful when working with [`Matrix`](/vault/Pipelines-main/matrix/) tasks that fan out with different parameter values.
...
When used with Matrix tasks, each matrix combination will create a separate `TaskRun` with the parameter values substituted appropriately in the Pod template. For more information and examples, see [Matrix Support with taskRunSpecs](/vault/Pipelines-main/pipelineruns/#matrix-support-with-taskrunspecs).
...
the [affinity assistant Pods](/vault/Pipelines-main/workspaces/#specifying-workspace-order-in-a-pipeline-and-affinity-assistants)
...
assistant Pod template [in your Tekton config](/vault/Pipelines-main/additional-configs/#customizing-basic-execution-parameters)
