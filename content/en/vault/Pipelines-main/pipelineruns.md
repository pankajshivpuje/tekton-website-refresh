...
A `PipelineRun` allows you to instantiate and execute a [`Pipeline`](/vault/Pipelines-main/pipelines/) on-cluster.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifying-the-target-pipeline) - Specifies the target [`Pipeline`](/vault/Pipelines-main/pipelines/).
- Optional:
  ...
  - [`taskRunSpecs`](#specifying-taskrunspecs) - Specifies a list of `PipelineTaskRunSpec` which allows for setting `ServiceAccountName`, [`Pod` template](/vault/Pipelines-main/podtemplates/), and `Metadata` for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-main/podtemplates/) to use as the basis for the configuration of the `Pod` that executes each `Task`.
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
in [Pipelines](/vault/Pipelines-main/pipelines/#tekton-bundles) or [TaskRuns](/vault/Pipelines-main/taskruns/#tekton-bundles).
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/vault/Pipelines-main/resolution/). The below example demonstrates
...
See more details in [Param.Enum](/vault/Pipelines-main/pipelines/#param-enum).
...
For more information, see [`ServiceAccount`](/vault/Pipelines-main/auth/).

**Note**: When using [Affinity Assistants](/vault/Pipelines-main/affinityassistants/), the Affinity Assistant pods automatically inherit
the `serviceAccountName` specified here, ensuring proper permissions in security-restricted environments like OpenShift.
See [Affinity Assistant ServiceAccount Configuration](/vault/Pipelines-main/affinityassistants/#serviceaccount-configuration) for details.

[`Custom tasks`](/vault/Pipelines-main/pipelines/#using-custom-tasks) may or may not use a service account name.
...
You can specify a [`Pod` template](/vault/Pipelines-main/podtemplates/) configuration that will serve as the configuration starting
...
[`Custom tasks`](/vault/Pipelines-main/pipelines/#using-custom-tasks) may or may not use a pod template.
...
wide `ServiceAccountName`  and [`podTemplate`](/vault/Pipelines-main/podtemplates/) configuration,
for example:
...
[Overriding `Task` `Steps` and `Sidecars`](/vault/Pipelines-main/taskruns/#overriding-task-steps-and-sidecars) for more information.
...
When using [`Matrix`](/vault/Pipelines-main/matrix/) to fan out `PipelineTasks`, the `taskRunSpecs` can reference matrix parameters for dynamic pod template configuration. Each matrix combination will create a separate `TaskRun` with the appropriate parameter values substituted in the pod template.
...
For a complete example, see [`pipelinerun-with-taskrunspecs-matrix-param-substitution.yaml`](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/pipelinerun-with-taskrunspecs-matrix-param-substitution.yaml).
...
- For information on mapping `Workspaces` to `Volumes`, see [Specifying `Workspaces` in `PipelineRuns`](/vault/Pipelines-main/workspaces/#specifying-workspaces-in-pipelineruns).
- For a list of supported `Volume` types, see [Specifying `VolumeSources` in `Workspaces`](/vault/Pipelines-main/workspaces/#specifying-volumesources-in-workspaces).
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1/pipelineruns/workspaces.yaml).

[`Custom tasks`](/vault/Pipelines-main/pipelines/#using-custom-tasks) may or may not use workspaces.
...
For more information, see the [`LimitRange` support in Pipeline](/vault/Pipelines-main/compute-resources/#limitrange-support).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/main/config/config-defaults.yaml).
...
- Required:
  ...
  - [`pipelineSpec`](/vault/Pipelines-main/pipelines/#configuring-a-pipeline) - The exact `PipelineSpec` used when starting the `PipelineRun`.
- Optional:
  - [`pipelineResults`](/vault/Pipelines-main/pipelines/#emitting-results-from-a-pipeline) - Results emitted by this `PipelineRun`.
  - `skippedTasks` - A list of `Task`s which were skipped when running this `PipelineRun` due to [when expressions](/vault/Pipelines-main/pipelines/#guard-task-execution-using-when-expressions), including the when expressions applying to the skipped task.
  - `childReferences` - A list of references to each `TaskRun` or `Run` in this `PipelineRun`, which can be used to look up the status of the underlying `TaskRun` or `Run`. Each entry contains the following:
    ...
    - [`whenExpressions`](/vault/Pipelines-main/pipelines/#guard-task-execution-using-when-expressions) - The list of when expressions guarding the execution of this task.
...
to a `TaskRun`, the complete [status of the `TaskRun`](/vault/Pipelines-main/taskruns/#monitoring-execution-status) and details
...
When a `PipelineRun` changes status, [events](/vault/Pipelines-main/events/#pipelineruns) are triggered accordingly.
...
When a `PipelineRun` has `Tasks` with [`when` expressions](/vault/Pipelines-main/pipelines/#guard-task-execution-using-when-expressions):
