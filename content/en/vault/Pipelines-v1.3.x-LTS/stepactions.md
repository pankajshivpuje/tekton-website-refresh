...
Like with `Tasks`, a `StepAction` must declare all the parameters that it uses. The same rules for `Parameter` [name](/vault/Pipelines-v1.3.x-LTS/tasks/#parameter-name), [type](/vault/Pipelines-v1.3.x-LTS/tasks/#parameter-type) (including [object](/vault/Pipelines-v1.3.x-LTS/tasks/#object-type), [array](/vault/Pipelines-v1.3.x-LTS/tasks/#array-type) and [string](/vault/Pipelines-v1.3.x-LTS/tasks/#string-type)) apply as when declaring them in `Tasks`. A `StepAction` can also provide [default value](/vault/Pipelines-v1.3.x-LTS/tasks/#default-value) to a `Parameter`.
...
Note that the `securityContext` from `StepAction` will overwrite the `securityContext` from [`TaskRun`](/vault/Pipelines-v1.3.x-LTS/taskruns/#example-of-running-step-containers-as-a-non-root-user).
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/vault/Pipelines-v1.3.x-LTS/resolution/). The below example demonstrates referencing a `StepAction` in git:
