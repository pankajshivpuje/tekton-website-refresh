...
We expose several kinds of exporters, including Prometheus, Google Stackdriver, and many others. You can set them up using [observability configuration](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/config/config-observability.yaml).
...
A sample config-map has been provided as [config-observability](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/config/config-observability.yaml). By default, taskrun and pipelinerun metrics have these values:
