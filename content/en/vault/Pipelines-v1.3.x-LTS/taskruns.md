...
- [Events](/vault/Pipelines-v1.3.x-LTS/events/#taskruns)
- [Running a TaskRun Hermetically](/vault/Pipelines-v1.3.x-LTS/hermetic/)
...
A `TaskRun` allows you to instantiate and execute a [`Task`](/vault/Pipelines-v1.3.x-LTS/tasks/) on-cluster. A `Task` specifies one or more
...
- Optional:
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-v1.3.x-LTS/podtemplates/) to use as
  ...
  - [`workspaces`](#specifying-workspaces) - Specifies the physical volumes to use for the
    [`Workspaces`](/vault/Pipelines-v1.3.x-LTS/workspaces/#using-workspaces-in-tasks) declared by a `Task`.
...
A working example can be found [here](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/examples/v1beta1/taskruns/no-ci/tekton-bundles.yaml).
...
the artifact adheres to the [contract](/vault/Pipelines-v1.3.x-LTS/tekton-bundle-contracts/). Additionally, you may also use the `tkn`
cli *(coming soon)*.
...
If a `Task` has [`parameters`](/vault/Pipelines-v1.3.x-LTS/tasks/#specifying-parameters), you can use the `params` field to specify their values:
...
See more details in [Param.Enum](/vault/Pipelines-v1.3.x-LTS/tasks/#param-enum).

### Specifying `Resource` limits

Each Step in a Task can specify its resource requirements. See
[Defining `Steps`](/vault/Pipelines-v1.3.x-LTS/tasks/#defining-steps). Resource requirements defined in Steps and Sidecars
...
- For information on mapping `Workspaces` to `Volumes`, see [Using `Workspace` variables in `TaskRuns`](/vault/Pipelines-v1.3.x-LTS/workspaces/#using-workspace-variables-in-taskruns).
...
- For an end-to-end example, see [`Workspaces` in a `TaskRun`](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/examples/v1/taskruns/workspace.yaml).
...
If the overridden `Task` uses a [`StepTemplate`](/vault/Pipelines-v1.3.x-LTS/tasks/#specifying-a-step-template), configuration on
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/config/config-defaults.yaml). If you set the global timeout to 0,
...
- Required:
  ...
  - [`taskSpec`](/vault/Pipelines-v1.3.x-LTS/tasks/#configuring-a-task) - `TaskSpec` defines the desired state of the `Task` executed via the `TaskRun`.
...
- Optional:
  ...
  - [`sidecars`](/vault/Pipelines-v1.3.x-LTS/tasks/#using-a-sidecar-in-a-task) - This field is a list. The list has one entry per `sidecar` in the manifest. Each entry represents the imageid of the corresponding sidecar.
...
When a `TaskRun` changes status, [events](/vault/Pipelines-v1.3.x-LTS/events/#taskruns) are triggered accordingly.
...
*More information on the inner workings of debug can be found in the [Debug documentation](/vault/Pipelines-v1.3.x-LTS/debug/)*
...
this is available as a [TaskRun example](https://github.com/tektoncd/pipeline/tree/release-v1.3.x/examples/v1/taskruns/run-steps-as-non-root.yaml).
