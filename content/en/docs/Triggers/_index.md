...
*  [`EventListener`](/docs/triggers/eventlisteners/) - listens for events at a specified port on your Kubernetes cluster.
   Specifies one or more `Triggers`.

*  [`Trigger`](/docs/triggers/triggers/) - specifies what happens when the `EventListener` detects an event. A `Trigger` specifies
   a `TriggerTemplate`, a `TriggerBinding`, and optionally, an `Interceptor`.

*  [`TriggerTemplate`](/docs/triggers/triggertemplates/) - specifies a blueprint for the resource, such as a `TaskRun` or `PipelineRun`,
...
*  [`TriggerBinding`](/docs/triggers/triggerbindings/) - specifies the fields in the event payload from which you want to extract
...
*  [`ClusterTriggerBinding`](/docs/triggers/triggerbindings/) - a cluster-scoped version of the `TriggerBinding`,
   especially useful for reuse within your cluster.

*  [`Interceptor`](/docs/triggers/interceptors/) - a "catch-all" event processor for a specific platform that
...
![TriggerFlow](https://raw.github.com/tektoncd/triggers/release-v0.34.x/docs/images/TriggerFlow.svg)
...
*   [Setting Up Tekton Triggers](/docs/triggers/install/)
...
*   [Troubleshooting Tekton Triggers](/docs/triggers/troubleshooting/)
