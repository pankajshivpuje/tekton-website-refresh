...
![](https://raw.github.com/tektoncd/chains/main/images/how-chains-works.png)
...
- Pipeline-level Provenance: The type hinting carrying the references of input/output artifacts can be defined either:
  - in the PipelineSpec
  - in the TaskSpec. However, in this case, the feature flag [`artifacts.pipelinerun.enable-deep-inspection`](/docs/chains/config/#pipelinerun-configuration) must be enabled to instruct Chains to dive deep into each child TaskRuns to look for type-hinting.
