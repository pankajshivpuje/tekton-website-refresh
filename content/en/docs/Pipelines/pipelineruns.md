...
- Optional:
  ...
  - [`taskRunSpecs`](#specifying-taskrunspecs) - Specifies a list of `PipelineTaskRunSpec` which allows for setting `ServiceAccountName`, [`Pod` template](/docs/pipelines/podtemplates/), and `Metadata` for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/docs/pipelines/podtemplates/) to use as the basis for the configuration of the `Pod` that executes each `Task`.
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
cluster's operator has installed. For more information including a tutorial, please check [resolution docs](/docs/pipelines/resolution/). The below example demonstrates
...
See more details in [Param.Enum](/docs/pipelines/pipelines/#param-enum).
...
**Note**: When using [Affinity Assistants](/docs/pipelines/affinityassistants/), the Affinity Assistant pods automatically inherit
the `serviceAccountName` specified here, ensuring proper permissions in security-restricted environments like OpenShift.
See [Affinity Assistant ServiceAccount Configuration](/docs/pipelines/affinityassistants/#serviceaccount-configuration) for details.

[`Custom tasks`](/docs/pipelines/pipelines/#using-custom-tasks) may or may not use a service account name.
...
[Overriding `Task` `Steps` and `Sidecars`](/docs/pipelines/taskruns/#overriding-task-steps-and-sidecars) for more information.
...
When using [`Matrix`](/docs/pipelines/matrix/) to fan out `PipelineTasks`, the `taskRunSpecs` can reference matrix parameters for dynamic pod template configuration. Each matrix combination will create a separate `TaskRun` with the appropriate parameter values substituted in the pod template.
...
For a complete example, see [`pipelinerun-with-taskrunspecs-matrix-param-substitution.yaml`](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/pipelineruns/pipelinerun-with-taskrunspecs-matrix-param-substitution.yaml).
...
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/pipelineruns/workspaces.yaml).
...
For more information, see the [`LimitRange` support in Pipeline](/docs/pipelines/compute-resources/#limitrange-support).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/config/config-defaults.yaml).
...
- Required:
  ...
  - [`pipelineSpec`](/docs/pipelines/pipelines/#configuring-a-pipeline) - The exact `PipelineSpec` used when starting the `PipelineRun`.
- Optional:
  - [`pipelineResults`](/docs/pipelines/pipelines/#emitting-results-from-a-pipeline) - Results emitted by this `PipelineRun`.
  - `skippedTasks` - A list of `Task`s which were skipped when running this `PipelineRun` due to [when expressions](/docs/pipelines/pipelines/#guard-task-execution-using-when-expressions), including the when expressions applying to the skipped task.
  - `childReferences` - A list of references to each `TaskRun` or `Run` in this `PipelineRun`, which can be used to look up the status of the underlying `TaskRun` or `Run`. Each entry contains the following:
    ...
    - [`whenExpressions`](/docs/pipelines/pipelines/#guard-task-execution-using-when-expressions) - The list of when expressions guarding the execution of this task.
