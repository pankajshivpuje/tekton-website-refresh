...
You also have the option to define a global Pod template [in your Tekton config](/docs/pipelines/additional-configs/#customizing-basic-execution-parameters) using the key `default-pod-template`.
...
- [Specifying a Pod template for a `TaskRun`](/docs/pipelines/taskruns/#specifying-a-pod-template)
- [Specifying a Pod template for a `PipelineRun`](/docs/pipelines/pipelineruns/#specifying-a-pod-template)

## Parameter Substitution in Pod Templates

When using Pod templates within `PipelineRun` [`taskRunSpecs`](/docs/pipelines/pipelineruns/#specifying-taskrunspecs), you can use parameter substitution to dynamically configure Pod template fields based on pipeline parameters. This is particularly useful when working with [`Matrix`](/docs/pipelines/matrix/) tasks that fan out with different parameter values.
...
When used with Matrix tasks, each matrix combination will create a separate `TaskRun` with the parameter values substituted appropriately in the Pod template. For more information and examples, see [Matrix Support with taskRunSpecs](/docs/pipelines/pipelineruns/#matrix-support-with-taskrunspecs).
...
assistant Pod template [in your Tekton config](/docs/pipelines/additional-configs/#customizing-basic-execution-parameters)
