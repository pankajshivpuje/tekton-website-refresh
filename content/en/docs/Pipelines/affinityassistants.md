...
> :seedling: **coschedule pipelineruns** and **isolate pipelinerun** modes are [**alpha features**](/docs/pipelines/additional-configs/#alpha-features).
...
configured for the `TaskRun` pods (i.e. other affinity rules specified in custom [PodTemplate](/docs/pipelines/pipelineruns/#specifying-a-pod-template) will be overwritten by Affinity Assistant).
If the `PipelineRun` has a custom [PodTemplate](/docs/pipelines/pipelineruns/#specifying-a-pod-template) configured, the `NodeSelector` and `Tolerations` fields will also be set on the Affinity Assistant pod. The Affinity Assistant
