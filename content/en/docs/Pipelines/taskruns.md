...
A working example can be found [here](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1beta1/taskruns/no-ci/tekton-bundles.yaml).
...
If a `Task` has [`parameters`](/docs/pipelines/tasks/#specifying-parameters), you can use the `params` field to specify their values:
...
See more details in [Param.Enum](/docs/pipelines/tasks/#param-enum).

### Specifying `Resource` limits

Each Step in a Task can specify its resource requirements. See
[Defining `Steps`](/docs/pipelines/tasks/#defining-steps). Resource requirements defined in Steps and Sidecars
...
- For information on mapping `Workspaces` to `Volumes`, see [Using `Workspace` variables in `TaskRuns`](/docs/pipelines/workspaces/#using-workspace-variables-in-taskruns).
...
- For an end-to-end example, see [`Workspaces` in a `TaskRun`](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/taskruns/workspace.yaml).
...
If the overridden `Task` uses a [`StepTemplate`](/docs/pipelines/tasks/#specifying-a-step-template), configuration on
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/config/config-defaults.yaml). If you set the global timeout to 0,
...
- Required:
  ...
  - [`taskSpec`](/docs/pipelines/tasks/#configuring-a-task) - `TaskSpec` defines the desired state of the `Task` executed via the `TaskRun`.
...
- Optional:
  ...
  - [`sidecars`](/docs/pipelines/tasks/#using-a-sidecar-in-a-task) - This field is a list. The list has one entry per `sidecar` in the manifest. Each entry represents the imageid of the corresponding sidecar.
...
this is available as a [TaskRun example](https://github.com/tektoncd/pipeline/tree/release-v1.9.x/examples/v1/taskruns/run-steps-as-non-root.yaml).
