...
Check the [Concepts](../concepts/) section to understand how Tasks, Pipelines,
and Runs fit together.

## Choose Your Path

{{< cards >}}
  {{< card link="pipelines" title="Build a Pipeline" icon="archive" subtitle="Chain multiple Tasks into a Pipeline that clones, tests, builds, and deploys." >}}
  {{< card link="pipelines-as-code" title="Pipelines as Code" icon="document-text" subtitle="Git-native CI/CD — define pipelines in a .tekton/ directory alongside your source." >}}
  {{< card link="tasks" title="Create Your First Task" icon="play" subtitle="Define a Task with steps, run it, and see the results — the fundamental building block." >}}
  {{< card link="triggers" title="Set Up Triggers" icon="lightning-bolt" subtitle="Automatically start Pipelines from Git push, PR events, or webhooks." >}}
  {{< card link="supply-chain-security" title="Supply Chain Security" icon="shield-check" subtitle="Sign and attest your build artifacts with Tekton Chains." >}}
{{< /cards >}}
...
- **Tekton Pipelines** installed — see the [Installation guide](../installation/)
...
2. **Create your first Task** — a Task defines one or more steps that run in
   containers. Start with the [Tasks tutorial](tasks/).
