...
function getBasePrefix () {
  ...
  var idx = p.indexOf('/docs/');
  if (idx === -1) idx = p.indexOf('/vault/');
...
function createLatestVersionNode (anchorNode, componentName, version, disabled, visible) {
  ...
  if (disabled) {
    ...
    latestVersionNode.setAttribute('href', `${prefix}/docs/${componentName.toLowerCase()}`);
...
function createPreviousVersionsNodes (anchorNode, componentName, previousVersions, excludedVersion) {
  ...
  previousVersions.slice().forEach(function (bundle) {
    ...
    previousVersionNode.setAttribute('href', `${prefix}/vault/${componentName.toLowerCase()}-${previousVersion.toLowerCase().replace(' ', '-')}`);
...
function setVaultVersionSwitcher () {
  ...
  componentVersions.forEach(function (componentVersion) {
    if (componentVersion.name.toLowerCase() === componentRawName) {
      ...
      componentVersion.tags.forEach(function (bundle) {
        if (bundle.displayName.toLowerCase().startsWith(versionRawName)) {
  ...
  if (currentLocationAnnouncerNode) {
  ...
  if (currentLocationAnnouncerTextNode) {
    currentLocationAnnouncerTextNode.innerHTML = `You are viewing the documentation of Tekton ${componentName} ${versionName || ''}`;
  }
  ...
  componentVersions.forEach(function (componentVersion) {
    if (componentVersion.name === componentName) {
      ...
      createLatestVersionNode(otherVersionsFormNode, componentName, latestVersion, false, true);
